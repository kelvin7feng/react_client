...
import { formatCount, navigateToArticle } from '../config/utils';
import { openChat } from '../config/chatManager';
import { RemoteImage } from './RemoteImage';
import { WaterfallArticleCard, WaterfallTwoColumnGrid } from './WaterfallArticleCard';
...
export default function UserProfile({
    ...
    const { data: homeData, isLoading: loading, error: queryError } = useUserHome(targetUserId, currentUserId);
    ...
    useEffect(() => {
        if (!homeData) return;
        setUserData(homeData.user);
        setUserStats(homeData.stats || { following_count: 0, followers_count: 0, likes_received: 0, favorites_received: 0 });
        setNotes(homeData.notes || []);
        setFollowed(!!homeData.followed);
    }, [homeData]);
    ...
    const handleArticleCardPress = useCallback((item: any) => {
        if (onArticlePress) {
            onArticlePress(item.id);
        } else {
            navigateToArticle(router, qc, item, currentUserId);
        }
    }, [onArticlePress, router, qc, currentUserId]);
                        ...
                        <WaterfallTwoColumnGrid
                            ...
                            renderItem={(item: any) => (
                                <WaterfallArticleCard
                                    item={item}
                                    onPress={() => handleArticleCardPress(item)}
